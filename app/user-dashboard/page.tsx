...
import { ProtectedPortal, RoleDebugCard } from "../../components/protected-portal";
...
function PassengerDashboardContent() {
  ...
  useEffect(() => {
    ...
    if (prototypeSession) setAccount(prototypeSession);
    else if (user) setAccount({ id: user.id, role: "user", name: user.fullName || user.primaryEmailAddress?.emailAddress || "Passenger", email: user.primaryEmailAddress?.emailAddress || "", phone: user.primaryPhoneNumber?.phoneNumber || "", password: "", createdAt: new Date().toISOString() });
    ...
    <main className="min-h-screen bg-rr-radial text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          ...
          <SignOutButton redirectUrl="/user-login"><button className="rounded-full border border-white/10 px-5 py-3 text-sm text-rr-silver hover:border-rr-purple/60">Log out</button></SignOutButton>
        </div>
        <div className="mt-6"><RoleDebugCard /></div>
        ...
        <div className="mt-4 grid gap-4">
          ...
          {myRequests.map((request, index) => <article key={`${request.createdAt}-${index}`} className="rr-card rounded-3xl p-6"><div className="text-xs uppercase tracking-[0.25em] text-rr-purple">{request.status}</div><h3 className="mt-3 text-2xl font-black">{request.experience}</h3><p className="mt-2 text-rr-chrome">{request.motorcycle}</p><p className="mt-2 text-sm text-rr-silver">{request.date} at {request.time} · {request.duration}</p><div className="mt-4 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]"><div className="rounded-2xl border border-white/10 bg-black/30 p-4 text-sm text-rr-silver"><strong className="text-white">Route:</strong><br />Pickup: {request.pickupLocation || "Not captured"}<br />Drop-off: {request.dropoffLocation || "Not captured"}<br />Preference: {request.routePreference || "Not captured"}<br />Estimate: {request.estimatedDistance || "Pending"} · {request.estimatedRideTime || "Pending"}</div><MockRouteMap pickup={request.pickupLocation} dropoff={request.dropoffLocation} compact /></div></article>)}
        </div>
      </section>
    </main>
  );
...
export default function UserDashboardPage() {
  return <ProtectedPortal requiredRole="passenger" title="Passenger portal restricted" description="Use the passenger login for ride reservations and passenger ride history."><PassengerDashboardContent /></ProtectedPortal>;
}

...
import { motorcycleInventory, riderProfiles } from "../../lib/prototype-data";
import { rideExperiences } from "../../lib/ride-experiences";
  ...
  return (
    <main className="min-h-screen bg-rr-radial text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        ...
        <p className="mt-4 max-w-3xl text-rr-chrome">Customers can choose a ride experience, pick a two-up capable motorcycle, then select an approved rider matched to that bike.</p>

        <h2 className="mt-12 text-2xl font-bold">Ride experiences and route packages</h2>
        <div className="mt-4 grid gap-4 lg:grid-cols-3">
          {rideExperiences.map((item) => (
            <article key={item.id} className="rr-card rounded-3xl p-6">
              ...
              <div className="mt-5 grid gap-2 text-xs text-rr-silver">
                <div className="rounded-2xl border border-white/10 bg-black/30 p-3">Destination: {item.destination}</div>
                <div className="rounded-2xl border border-white/10 bg-black/30 p-3">Pricing: {item.price} · {item.priceMode}</div>
                <div className="rounded-2xl border border-white/10 bg-black/30 p-3">Guardrail: {item.duration} · {item.mileageCap}</div>
              </div>
          ...
          {motorcycleInventory.map((bike) => (
            <article key={bike.id} className="rr-card overflow-hidden rounded-3xl">
              ...
              <div className="p-6">
                ...
                <a href={bike.vendorUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex text-sm text-rr-purple">Open vendor gallery/source →</a>
              </div>
            </article>
          ))}
        </div>
